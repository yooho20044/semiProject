...
public class Admin_memberService {
	...
	public ArrayList<MemberInfoDTO> printMember(int currentPage){ //회원 전체 내역 페이징 처리를 위한 메서드
		int startRange = currentPage * recordCntPage - (recordCntPage-1);
		int endRange = currentPage * recordCntPage;
		try {
			ArrayList<MemberInfoDTO> list = dao.getMemberList(startRange, endRange);
			return list;
		}catch(Exception e) {
			e.printStackTrace();
		}return null;
	...
	public ArrayList<MemberInfoDTO> getUserByIdOrNickname(String id, int currentPage){//회원 아이디/닉네임 검색 내역 페이징 처리를 위한 메서드
		int startRange = currentPage * recordCntPage - (recordCntPage-1);
		int endRange = currentPage * recordCntPage;
		try {
			ArrayList<MemberInfoDTO> list = dao.getUserByIdOrNickname(id, startRange, endRange);
			return list;
		}catch(Exception e) {
			e.printStackTrace();
		}return null;
	...
	public ArrayList<MemberInfoDTO> getUserByEmail(String value, int currentPage){//회원 이메일 검색 내역 페이징 처리를 위한 메서드
		int startRange = currentPage * recordCntPage - (recordCntPage-1);
		int endRange = currentPage * recordCntPage;
		try {
			ArrayList<MemberInfoDTO> list = dao.getUserByEmail(value, startRange, endRange);
			return list;
		}catch(Exception e) {
			e.printStackTrace();
		}return null;
	...
	public ArrayList<MemberInfoDTO> getUserByPhone(String id, int currentPage){//회원 휴대폰 검색 내역 페이징 처리를 위한 메서드
		int startRange = currentPage * recordCntPage - (recordCntPage-1);
		int endRange = currentPage * recordCntPage;
		try {
			ArrayList<MemberInfoDTO> list = dao.getUserByPhone(id, startRange, endRange);
			return list;
		}catch(Exception e) {
			e.printStackTrace();
		}return null;
